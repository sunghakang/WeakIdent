...
def build_dictionary_list(lhs_idx: np.ndarray, betas: list,
                          alphas: list) -> np.array:
    """
    build a list of dictionary including the left hand side feature such as u_t and v_t

    Args:
        lhs_idx (np.ndarray): the tag of left hand side feature (dynamic variable)
    ...
    dict_list = np.block([[dict_list], [lhs_idx]])
...
def get_tags_v2(
    ...
    dict_list = build_library_tags(n, dim, max_dx, max_poly, use_cross_der,
                                   lhs_idx)
    lhs_ind = np.zeros(lhs_idx.shape[0])
    for i in range(lhs_idx.shape[0]):
        lhs_ind[i] = np.flatnonzero(npi.contains([lhs_idx[i, :]], dict_list))
    lhs_ind = lhs_ind.astype(int)
    dict_list_without_lhs = np.delete(dict_list, lhs_ind, 0)
    c_true = np.zeros((dict_list.shape[0] - n, lhs_idx.shape[0]))
    for i in range(n):
    ...
    return dict_list, lhs_ind, c_true
...
def build_tag_pde(n: int, dim_x: int, dict_list:np.ndarray, lhs_ind: np.ndarray) -> Tuple[list, list]:
    """
    ...
    """
    base_names = [['','u'] + ['u^' + str(i) for i in range(2, 10)],  ['','v'] + ['v^' + str(i) for i in range(2,10)]]
...
def build_tag_ode(n: int, dict_list:np.ndarray, lhs_ind: np.ndarray) -> Tuple[list, list]:
    """
    ...
    Returns:
        ...
        tags_rhs(list): a list of string tag for rhs feature(s)
    """
    base_names = [['','x'] + ['x^' + str(i) for i in range(2, 10)],  ['','y'] + ['y^' + str(i) for i in range(2,10)], \
    ...
    for i in range(n):
        ...
        for l in range(c.shape[0]):
            ...
            if tags_rhs[l] == '1' :
                ...
                eqn_str += ' ' + ["-", "+"][coe > 0] + ' ' +str(round(abs(coe),3)) + ' ' + tags_rhs[l]
